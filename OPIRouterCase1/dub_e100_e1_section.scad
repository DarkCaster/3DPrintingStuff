...
)
{
	...
	gpio_cut_size=[size[0]-4*wall_sz-2*screw_diam-opi_clearance*2,(size[1]-2*wall_sz-dub_holder_cut2_size[1])/2-opi_clearance*2];
	gpio_cut_move=[0,-(size[1]-gpio_cut_size[1])/2+wall_sz+opi_clearance,-attach_clearance];
			...
			difference()
			{
				union()
				{
					...
					case_section(size=[size_x,size_y,height],
						...
						attach=attach,
						quality=quality,
						vents=[true,use_back_layout?true:false,true,use_back_layout?false:true],
						vent_size=vent_size,
						vent_period=vent_period,
						vent_angle=vent_angle,
						center_xy=true);
					//draw dub e100/e1 holder
					rotate(a=use_back_layout?180:0,v=[0,0,1])
