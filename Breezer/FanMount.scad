...
(
	...
	int_clip_head_size=[8,4],
	int_clip_diff=[110,110],
	quality=2
)
{
	cutClr=1;
	...
	difference()
	{
		union()
		{
			...
			cube_vround(size=[int_clip_size[0]+cutClr,int_clip_size[1]+cutClr,ext_size[2]],center_xy=true,rounding=int_rounding,quality=quality,round_corners=[i<0&&j<0, i<0&&j>0, i>0&&j>0, i>0&&j<0]);
		}
		for (i=[-1:2:1],j=[-1:2:1])
